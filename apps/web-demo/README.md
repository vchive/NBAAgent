...
- API 可用时，`api-client.js` 探测 `/healthz`，聊天走 `POST /api/v1/chat/stream`，左栏
  赛事焦点走 `GET /api/v1/highlights`；“精彩回顾”默认请求
  `GET /api/v1/highlights/recent?limit=5`，自定义时间请求
...
仍会执行安全检查、事实核验和输出守卫，模型不可用时显示“模型回退 · 已核验事实”。

左栏的赛事焦点支持单日多场比赛和历史范围：主推卡展示当前选中场次，下面的比赛列表
  会列出接口返回的全部 `games[]`。点击任一卡片会同步右侧 HUD、节次比分和文字 PBP；
  后续聊天请求会自动携带该卡片 ID，服务端将“这场”及匹配对阵问题绑定到当前比赛；
