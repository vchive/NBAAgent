...
# Read a Baidu Qianfan AI Search key without echoing it and atomically write a
...
SECRET_FILE="$SECRET_DIR/qianfan_search_api_key"
TEMP_FILE=""
TOKEN=""
FORCE=false
...
while (($# > 0)); do
    case "$1" in
        --force) FORCE=true ;;
        --help|-h)
            echo "用法：scripts/configure-qianfan-search-key.sh [--force]" >&2
            exit 0
            ;;
...
if [[ -e "$SECRET_FILE" && "$FORCE" != true ]]; then
    printf 'qianfan_search_api_key 已存在，覆盖它吗？[y/N] ' >&2
    IFS= read -r answer
...
printf '百度千帆 AI Search API key（输入不回显）： ' >&2
IFS= read -r -s TOKEN
printf '\n' >&2
...
TEMP_FILE="$(mktemp "$SECRET_DIR/.qianfan_search_api_key.XXXXXX")"
chmod 600 -- "$TEMP_FILE"
printf '%s\n' "$TOKEN" > "$TEMP_FILE"
...
echo "下一步：make deploy-live-qianfan。" >&2
