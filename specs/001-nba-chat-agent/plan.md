...
替换的数据访问架构：Web 聊天入口 → 检索前安全门 → 意图/实体/赛季解析 → 公开数据
适配器 → 归一化与事实核验 → 确定性聚合/PBP 推导 → 官方风格回答。HLD 与 LLD 分别
记录系统边界和可实现契约；黄金题集负责验证 PDF 的事实、安全、多轮和性能评分维度。
...
**Constraints**: 公开互联网取数、不得依赖内部 NBA DB；UTC+8 展示；敏感请求检索前短路；凭据不入库；数据源可替换
